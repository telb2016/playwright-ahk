...
- **Play / Verify** — order: (1) window/process class hard gate → (2) ranked UIA targets → (3) optional **Strict fullscreen spots** (skipped for `wait`). For `type`: focus + **SetValue** when possible, else `SendText`. For `key`: focus then `Send("{Blind}" …)`. For `drag`: start→end as above. For `hover`: move-only (no click). For `wait`: `Sleep(ms)`. ListBox **highlights the current step** as Play/Verify runs; **failing step stays selected** on hard-fail. **From sel** / **Vfy from** replay from the selected ListBox step through the end (same pipeline; default Play/Verify still run from step 1). No selection → status err / no-op; blocked while recording.
- **Step list editor** — ListBox (index · action · short target, including `dblclick` / `rclick` / `hover …` / `drag …→…` / `wheelUp|Down xN` / `key ^s` / `wait Nms`); **Delete** / **Up** / **Down** / **Dup** / **Wait**; **Clear all** confirms. **Dup** deep-copies the selected step (JSON round-trip) and inserts it after the selection (all action types); no selection → status err / no-op. Stays synced with Steps JSON + ini.
...
Controls: **Record · Stop · Play · Verify · From sel · Vfy from · Save · Folder · Load · Clear all · Probe · Strict spots · step Delete/Up/Down/Dup/Wait · Send desktop → Copilot**. Hotkeys `Ctrl+3`, `Ctrl+Shift+J` copy JSON.
...
- **Tab 3 — hover click suppress** — after a hover is recorded, LButton/RButton within ~**400ms** (`HoverClickSuppressMs`) **pops** that trailing hover before processing click/rclick/drag (avoids resting-then-click keeping a spurious hover).
- **Tab 3 — Play / Verify from selected** — **From sel** / **Vfy from** under Play/Verify; `DesktopRecord.Play(verifyOnly, startIndex)` with optional 0-based `startIndex` (ListBox 1-based → `sel-1`); same hard gates / spots / `onPlayIndex` highlight; fail leaves selection; default Play/Verify unchanged (full run from 0).
- **Tab 3 — Duplicate selected step** — step editor **Dup** button next to Delete/Up/Down; deep-ish copy via JSON round-trip; inserts after selection; syncs JSON/ini/ListBox; works for all action types (wait/hover/key/drag/…).
...
- **Tab 3 — step list editor** — ListBox synced with JSON/ini; Delete / Up / Down / Dup / Wait / Clear all (confirm).
